...
def mmilp_call(C, A, P, R, w1, w2, u, outfile):
	...
	#	for j in range(i, N):
	#		if C[i][j] > 0 or i == j:
	#			print('z'+str(i)+str(j)+'+',file=out,end='')
	#print('z'+str(N-1)+str(N-1)+'), GRB.MINIMIZE)\n',file=out,end='')
	...
	#	for j in range(i, N):
	#		if C[i][j] > 0 or i == j:
	#			print('\tprint(str(z'+str(i)+str(j)+'.x)+\',\', file=ofid, end=\'\')\n',file=out,end='')
	#print('\tprint(str(z'+str(N-1)+str(N-1)+'.x)+\'\\n\', file=ofid, end=\'\')\n',file=out,end='')
...
for i in range(graph_sum):
	...
	if len(node_vec) == 1:
		...
		#trim
		#if len(node_vec[0]) < 200 or C[0][0] < 1:
		#	continue;
		#if len(node_vec[0]) < 500 and C[0][0] < ave_cov * 0.5 and C[0][0] < 12:
		#	x5 = x5 + 1
		#	continue;
		#if len(node_vec[0]) < 800 and C[0][0] < ave_cov*0.1 and C[0][0] < 8:
		#	x8 = x8 + 1
		#	continue;
	...
	if satisfied_sum > w*len(node_vec) and len(P) < 10:
		...
		for j in range(len(P)):			
			...
			for k in range(len(P[j])):
				node_id = P[j][k]
				if k > 0:
					trans_seq = trans_seq + node_vec[node_id]
				else:
					trans_seq = node_vec[node_id]	
			if len(trans_seq) > 0:
				trans_seq = trans_seq + '\n'
	...
	for j in range(len(results_vec)):
		...
		#if results_vec[j] == '1.0' and cov_vec[j] >= min_cov*0.5:	
